...
// Dragging a window between two monitors with different DPIs
// will emit a resize event
...
export declare interface XpraClient {
    on(event: "connected", callback: () => any | Promise<any>): void
    on(event: "disconnected", callback: (reason: any) => any | Promise<any>): void

    on(event: "windowCreate", callback: (data: {
        wid: number,
        x: number,
        y: number,
        width: number,
    ...
    on(event: "windowClose", callback: (wid: number) => any | Promise<any>): void
    on(event: "windowRaise", callback: (wid: number) => any | Promise<any>): void
    on(event: "windowIcon", callback: (data: {
        wid: number,
        width: number,
        height: number,
        data: string}) => any | Promise<any>): void
    on(event: "windowResize", callback: (data: {
        wid: number,
        width: number,
        height: number
    }) => any | Promise<any>): void
    on(event: "windowMove", callback: (data: {
        wid: number,
        x: number,
        y: number
    }) => any | Promise<any>): void
    on(event: "windowDraw", callback: (data: {
        wid: number,
        x: number,
        y: number,
        srcWidth: number,
        srcHeight: number,
        encoding: unknown // probs string or number primitive
        data: any,
        seqId: number,
        rowStride: any,
        options: any
    ...
    on(event: "pointerMove", callback: (data: {
        wid: number,
        x: number,
        y: number
    }) => any | Promise<any>): void

    on(event: "desktopSize", callback: (data: {
        width: number,
        height: number,
        maxWidth: number,
        maxHeight: number
    ...
    on(event: "endStream", callback: (wid: number) => any | Promise<any>): void

    on(event: "cursor", callback: (data: {
        width: number,
        height :number,
        hotX: number,
        hotY: number,
    ...
    on(event: "bell", callback: () => any | Promise<any>): void

    on(event: "encodings", callback: (encodings: unknown) => any | Promise<any>): void
    on(event: "sound", callback: (data: {
        codec: string,
        data: any,
        options: any,
        metadata: any
    }) => any | Promise<any>): void
    on(event: "draw", callback: () => any | Promise<any>): void
    on(event: "xdgMenu", callback: (evt: any) => any | Promise<any>): void

    on(event: "serverError", callback: (data: {code: any, reason: any}) => any | Promise<any>): void
    on(event: "ready", callback: () => any | Promise<any>): void
    on(event: "ping", callback: (data: { clientLatency: number, serverLatency: number, loadAvg: number }) => any | Promise<any>): void
    on(event: "xdgMenu", callback: () => any | Promise<any>): void
    on(event: "xdgMenu", callback: () => any | Promise<any>): void
    on(event: "xdgMenu", callback: () => any | Promise<any>): void
    on(event: "xdgMenu", callback: () => any | Promise<any>): void
    on(event: "xdgMenu", callback: () => any | Promise<any>): void
    ...
    on(event: "windowMetadata", callback: (data: {wid: number, metadata: unknown}) => any | Promise<any>): void
}
...
export class XpraClient {
    ...
    private emit(evt: string, ...args) {
        console.log("XPRA EMIT", evt, args);

        if (this.handlers[evt])
            this.handlers[evt].forEach(h => h(args));
    }
                ...
                case 'error':
                    {
                        ...
                        this.emit("serverError", {code, reason})
                        this.close()
                    }
                    break
                case 'challenge':
                ...
                case 'ping_echo':
                    {
                        ...
                        this.emit("ping", { clientLatency, serverLatency, loadAvg });
                    }
                    break
                case 'info-response':
                    {
                ...
                case 'new-tray':
                    {
                        ...
                        console.log('creating new tray window:', 'wid', wid, 'x', x, 'y', y,
                            'w', w, 'h', h, 'metadata', metadata)
                        // TODO: Create a tray window

                        this.emit("windowCreate", {
                            wid: wid,
                            x,
                            y,
                            width: w,
                ...
                case 'new-window':
                    {
                        ...
                        const props = obj[7] || {}
                        console.log('server creating window:', 'wid', wid, 'x', x, 'y', y,
                ...
                case 'new-override-redirect':
                    {
                        ...
                        const metadata = obj[6]
                        const props = obj[7] || {}
                        console.log('creating override-redirect window:', 'wid', wid, 'x', x, 'y', y,
                            'w', w, 'h', h, 'metadata', metadata, 'props', props)
                        // this.$createWnd(wid, x, y, w, h, metadata, props, true)
                ...
                case 'window-metadata':
                    {
                        ...
                        this.emit("windowMetadata", { wid: wid, metadata });
                    }
                    break
                case 'lost-window':
                    {
                        const wid = obj[1];
                        this.emit("closeWindow", wid);
                    }
                    break
                case 'raise-window':
                    {
                        const wid = obj[1];
                        this.emit("windowRaise", wid);
                    }
                    break
                case 'window-icon':
                    {
                ...
                case 'initiate-moveresize':
                    {
                        ...
                        this.actingMoveResize = { wid, x, y, direction, button, source };
                    }
                    break
                ...
                case 'desktop_size':
                    {
                        ...
                        const maxW = obj[3];
                        const maxH = obj[4];
                        this.emit("desktopSize", {width: w, height: h, maxWidth: maxW, maxHeight: maxH});
                    }
                    break
                case 'eos':
                    {
                        const wid = obj[1];
                        this.emit("endStream", wid);
                    }
                    break
                case 'draw':
                    {
                ...
                case 'sound-data':
                    {
                        ...
                        this.emit("sound", {codec, data, options, metadata});
                    }
                    break
    ...
    close() {
        ...
        this.emit("disconnected", "Socket was closed by client");
    }
     ...
     */
    sendDecoderCallback(seqId, wid, w, h, start, message = '') {
        const decodeTime = Math.round((performance.now() - start) * 1000) // seconds
        if (this.connected)
            this.protocol.send(['damage-sequence', seqId, Number(wid), w, h, decodeTime, message])
    }
     ...
     * @param {Number} wid 0 for no window
     * @param {Object} windows wid as key, window object as value
     */
    pointerMoveEvent(e, wid, windows) {
        ...
        const posX = Math.round(e.offsetX + windows[wid]?.x || 0)
        const posY = Math.round(e.offsetY + windows[wid]?.y || 0)

        const p = this.actingMoveResize as any
        if (p.direction == DIRECTION_MOVE) {
            if (this.draggingOffsetX === null) {
                this.draggingOffsetX = p.x - windows[p.wid].x
                this.draggingOffsetY = p.y - windows[p.wid].y
            }

            this.emit("windowMove", {
                wid: p.wid, 
                x: posX - this.draggingOffsetX, 
                y: posY - this.draggingOffsetY
            });
            return;
        }

        // TODO: configure max FPS?
        ...
        this.protocol.send(['pointer-position', Number(wid), [posX, posY],
     ...
     * @param {Object} windows wid as key, window object as value
     */
    pointerActionEvent(e, pressed, wid, windows) {
        if (e.offsetX < 0 || e.offsetY < 0)
            return // Throttle
        ...
        const p: any = this.actingMoveResize;
        if (p.direction == DIRECTION_MOVE && !pressed) {
            const wnd = windows[p.wid]
            this.moveWnd(p.wid, wnd.x, wnd.y, wnd.w, wnd.h, toPlainObject(wnd.props))
        ...
        const posX = Math.round(e.offsetX + (windows[wid]?.x || 0))
        const posY = Math.round(e.offsetY + (windows[wid]?.y || 0))
        console.log(pressed ? '⇩' : '⇧', 'wid', wid, 'button', button, 'x', posX, 'y', posY)
        ...
        setTimeout(() => {
            this.protocol.send(['button-action', Number(wid), button, pressed, [posX, posY],
                this.$convertModifiers(e), [],
            ])
        }, CLIPBOARD_DELAY)
    }
     ...
     * @param {Number} y
     */
    releaseButtons(e, wid = 0, x = 0, y = 0) {
        if (this.serverCaps['readonly'] || !this.connected)
            return

        for (const button of this.pressedButtons) {
            console.log("releasing stuck button:", button)
            this.protocol.send(['button-action', Number(wid), button, false, [x, y],
                this.$convertModifiers(e), [],
            ])
        }
        this.pressedButtons = []
     ...
     * @param {Number} wndY
     */
    wheelEvent(e, wid = 0, wndX = 0, wndY = 0) {
        if (this.serverCaps['readonly'] || !this.connected)
            return
        ...
        if (this.serverCaps['wheel.precise']) {
            if (px != 0)
                this.protocol.send(['wheel-motion', Number(wid),
            ...
            while (Math.abs(this.remainingDeltaX) >= 120) {
                this.protocol.send(['button-action', Number(wid),
    ...
    focusWnd(focusedWndId, unfocusedWndIds, x, y, w, h, props) {
        ...
        for (const wid of unfocusedWndIds) {
            this.protocol.send(['configure-window', Number(wid), x, y, w, h,
    ...
    /**
     * Suspend windows
     * @param {Array} wids window id list
     */
    suspend(wids) {
        if (this.connected)
            this.protocol.send(['suspend', true, wids.map(t => Number(t))])
    }
    /**
     * Resume windows
     * @param {Array} wids window id list
    ...
    resume(wids) {
        if (this.connected)
            this.protocol.send(['resume', true, wids.map(t => Number(t))])
    }
    /**
     * Refresh window
     * @param {Number} wid the window id to refresh. -1 for all.
     */
    refreshWnd(wid = -1) {
        if (this.serverCaps['readonly'] || !this.connected)
            return
        this.protocol.send(['buffer-refresh', Number(wid), 0, 100, {
            'refresh-now': true,
            'batch': { reset: true },
        }, {
                /* Client properties */
    ...
    closeWnd(wid) {
        if (this.serverCaps['readonly'] || !this.connected)
            return
        this.protocol.send(['close-window', Number(wid)]) // trigger onWndClose
    }
